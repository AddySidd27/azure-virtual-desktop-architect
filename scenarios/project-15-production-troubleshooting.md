...
**What this engagement actually taught.** The technical root cause, a known application memory leak on a subset of hosts running a stalled-partway-through image rollout, was, in isolation, a fairly ordinary finding, the kind covered by existing troubleshooting patterns in this book. What made this engagement genuinely instructive was everything around the technical finding: reconstructing an accurate picture from three days of uncoordinated changes, holding a structured method against real pressure to skip ahead, and confirming that two of the customer's own well-intentioned prior changes had fixed nothing and needed to be reverted, not just left alone. A live incident engagement tests discipline under pressure at least as much as it tests technical knowledge.

**What would be done differently with more time.** Layer 6 (host pool configuration, specifically the stalled rolling image update) was found by manually cross-referencing the affected-host list against Northgate's own deployment history, a process that worked here but relied on Northgate's deployment history being complete enough to cross-reference, which it was, somewhat by luck. A more systematic approach, tagging session hosts with their golden image version as a queryable property from day one of any engagement, not discovered as useful only during this specific incident, would make this cross-reference faster and more reliable on a future engagement where the deployment history is less complete.
