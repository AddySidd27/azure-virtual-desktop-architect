...
**What this engagement actually taught.** Governance at this scale is not a bigger version of a single-environment build. The hardest problems were organisational, not technical: convincing four business units that had built independently for two years to converge on a shared standard without losing the autonomy that made them build independently in the first place. Every technical decision in this project, subscriptions over resource groups, pinned module versions, selective PIM, was chosen specifically because it preserved business-unit autonomy while still closing the audit finding and the cost-visibility gap. A more centrally convenient design existed for almost every decision here and was rejected because it would have failed politically even if it succeeded technically.

**What would be done differently with more time.** The tag back-fill script was built and tested against Automotive first, then adapted for the other three, a shared, parameterised script from the start would have saved roughly a week. Materials' interim standing-access exception, while properly time-boxed, revealed that the PIM migration plan hadn't accounted for in-flight application migrations at all four business units, only at the one it was designed around; a pre-migration survey of in-flight work at each business unit would have caught this before it became a live exception request.
